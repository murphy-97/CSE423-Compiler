...
def build_block(ast, block_root, global_vars, func_params, function, builder):
    ...
    traversal_queue = []    # Used to traverse tree build node_queue (excludes subblocks)
    node_queue = []         # Used to build function. Nodes queued in modified post order
    traversal_queue.append(block_root)

    while (len(traversal_queue) > 0):
        iter_node = traversal_queue.pop(0)   # Get next node from traversal_queue
        node_queue.append(iter_node)

        child_list = []

        # Add children to the traversal_queue
        if (iter_node.tag not in ['if', 'while', 'block'] and iter_node.tag not in __type_dict):
            ...
            child_list.append(ast.children(iter_node.identifier)[0])

        traversal_queue = child_list[::-1] + traversal_queue # Prepend children in reverse order

    node_queue = node_queue[::-1]   # Reverse queue

    # Use modified post order traversal to build function by pop(0)ing node_queue
    ...
    # Begin modified post order traversal of parse tree
            ...
            # Can be ignored in function building
                ...
                if (is_numeral):
                    ...
                    if (iter_node.tag in func_locals):
                        ...
                        # Function argument that has not been converted
                        result = function.args[
                            [p[1] for p in func_params].index(iter_node.tag)
                        ]

                    else:
                        # Global var or undeclared var (might be in symbol table)
                        result = get_variable(
                            iter_node.tag,
                            func_locals,
                            func_params,
